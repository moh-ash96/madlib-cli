...
for i in range(len(actual_parts)):
    response = input(f"{actual_parts[i]} >")
...
contents = merge(actual_stripped, t2)
print(contents)

with open ("assets/make_me_a_video_game_template.copy.txt", "w") as new:
    new.write(contents)
